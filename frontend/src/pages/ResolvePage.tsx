  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (scope) {
        ...
        list = [await api.resolve(variable, server, scope)];
      } else {
        // No override: show every candidate value (one box per scope).
        list = await api.resolveAll(variable, server);
      }
  ...
  return (
    <div>
      ...
      <p className="subtle">
        ...
        source). An exact scope match overrides the global default. Optionally narrow by context, or
        pick a specific scope to resolve directly.
      </p>
      <form className="form inline" onSubmit={onSubmit}>
        ...
        <div className="field">
          ...
          <Combobox
            ...
            value={scope}
            options={Array.from(new Set(scopes.map((s) => String(s["name"]))))}
            onChange={setScope}
          />
        </div>
      ...
      {results && (
        results.length > 1 ? (
          <div className="result-grid">
            {results.map((r, i) => (
              <ResultCard key={r.valueId ?? i} result={r} showScope status={statuses?.[keyOf(r)]} />
            ))}
          </div>
        ) : (
